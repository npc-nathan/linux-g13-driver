...
`built-in` (cpu, memory, uptime, time, media, profile, keys), `env:`, `file:`, `json:`, `cmd:`, `regex:`, `mqtt:`, `ws:`, `imap:` and
`http:`.
...
`env:NAME`, `file:PATH`, `cmd:COMMAND`, `json:PATH#FIELD`, `regex:PATH#PATTERN`, `mqtt:<endpoint>/<topic>#field`, `ws:<endpoint>#field`, `imap:<endpoint>/<mailbox>#field` and `http:<endpoint>/<path>#field` for
