...
`built-in` (cpu, memory, uptime, time, media, profile, keys), `env:`, `file:`, `json:`, `cmd:`, `regex:`, `mqtt:`, `ws:` and
`http:`.
...
`env:NAME`, `file:PATH`, `cmd:COMMAND`, `json:PATH#FIELD`, `regex:PATH#PATTERN`, `mqtt:<endpoint>/<topic>#field`, `ws:<endpoint>#field` and `http:<endpoint>/<path>#field` for
